...
// MARK: - Alphabet-indexed horizontal row
...
///
/// The scrubber bar sits above the scroll view and fades in on
/// hover. Hovering a letter scrolls the row so the first item
/// whose first-letter bucket matches that letter is leading-edge.
...
/// represented letters.
private struct AlphabetIndexedRow<Item: Identifiable, Content: View>: View
...
/// width stays stable as filter results change. Hover (not click)
/// scrubs — matches macOS Finder's column scroll-bar feel.
private struct AlphabetBar: View {
    let alphabet: [Character]
    let presentLetters: Set<Character>
    let onHoverLetter: (Character) -> Void
        ...
        HStack(spacing: 1) {
            ForEach(alphabet, id: \.self) { letter in
                Text(String(letter))
                    ...
                    .foregroundStyle(presentLetters.contains(letter) ? .secondary : .tertiary)
                    .frame(width: 12, height: 16)
                    .contentShape(Rectangle())
                    .onHover { isHovered in
                        // Only fire scroll for letters that actually
                        // have items — letting absent letters scrub
                        // would feel jumpy as the user drags across.
                        if isHovered && presentLetters.contains(letter) {
